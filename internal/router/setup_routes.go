...
func SetupRoutes(cfg *config.Config, authHandler *handlers.AuthHandler, homeHandler *handlers.HomeHandler, taskHandler *handlers.TaskHandler, billHandler *handlers.BillHandler, roomHandler *handlers.RoomHandler, homeRepo repository.HomeRepository) http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Route("/room", func(r chi.Router) {
			r.With(middleware.RequireAdmin(homeRepo)).Post("/create", roomHandler.Create)
			r.With(middleware.RequireMember(homeRepo)).Get("/{room_id}", roomHandler.GetByID)
			r.With(middleware.RequireMember(homeRepo)).Get("/home/{home_id}", roomHandler.GetByHomeID)
			r.With(middleware.RequireAdmin(homeRepo)).Delete("/{home_id}/{room_id}", roomHandler.Delete)
		})
		...
		r.Route("/task", func(r chi.Router) {
			...
			r.With(middleware.RequireMember(homeRepo)).Patch("/reassign_room", taskHandler.ReassignRoom)
		})
